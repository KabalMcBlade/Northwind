...
bool VertexBufferObject::Alloc(const VkDevice& _device, const void* _data, size _allocSize, EBufferUsage _usage)
{
...
void VertexBufferObject::ReferenceTo(const VertexBufferObject& _other)
{
...
void VertexBufferObject::ReferenceTo(const VertexBufferObject& _other, size _refOffset, size _refSize)
{
    nwAssertReturnVoid(!IsMapped(), "Buffer must be not mapped");
...
void VertexBufferObject::Update(const void* _data, size _size, size _offset /*= 0*/) const
{
...
void* VertexBufferObject::MapBuffer(EBufferMappingType _mapType, size _offset)
{
