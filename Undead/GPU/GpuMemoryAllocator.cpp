...
GpuMemoryAllocator::GpuAllocator* GpuMemoryAllocator::GetAllocator()
{
	static eos::HeapAreaR memoryArea(SettingsDefines::Engine::kGPUAllocatorSize);
	static GpuAllocator memoryAllocator(memoryArea, "GPUAllocator");
...
void GpuMemoryAllocator::Shutdown()
{
	...
	{
		eos::Vector<GpuMemoryList*, GpuAllocator, GetAllocator>& memoryList = m_gpuMemory[i];
...
GpuMemoryAllocation GpuMemoryAllocator::Alloc(const GpuMemoryCreateInfo& _createInfo)
{
	...
	for (size i = 0; i < count; ++i)
	{
		...
		if (node->GetMemoryTypeIndex() != memoryTypeIndex)
		{
			continue;
		}
	...
	VkDeviceSize logicSize = (_createInfo.m_usage == EMemoryUsage_GPU) ? SettingsDefines::Engine::kGpuDeviceLocalSize : SettingsDefines::Engine::kGpuHostVisibleSize;
...
void GpuMemoryAllocator::Free(GpuMemoryAllocation& _gpuMemory)
{
	...
	if (_gpuMemory.m_owner->GetAllocatedDeviceSize() == 0)
	{
		eos::Vector<GpuMemoryList*, GpuAllocator, GetAllocator>& memoryList = m_gpuMemory[_gpuMemory.m_owner->GetMemoryTypeIndex()];
